...
export function useFilteredReports(filters: TriageFilters, reports: Report[] = []) {
  return useMemo(() => {
    return reports
      ...
      // Newest first. IDs are chronological (Mongo ObjectIds sort by
      // creation time; mock IDs increment sequentially), so this keeps
      // recent reports on top without needing a timestamp field.
