...
Architecture (chosen for this data, not for novelty):
  ...
  action   : multiclass SGD log-loss classifier  (single-label, 18 classes)
  object   : multiclass SGD log-loss classifier  (single-label, 19 classes)
  controls : one binary SGDClassifier per deficiency with >= MIN_POSITIVES
             weak positives - used ONLY to raise review candidates, never to
             assert a deficiency (see src/pipeline.py)
  sif      : binary SGDClassifier on the explainable scorer's confident
...
HUMAN_WEIGHT = 100.0  # human seed rows are worth more than weak labels
MIN_CLASS_ROWS = 2
MIN_POSITIVES = 8
...
) -> tuple[SGDClassifier | None, dict[str, object]]:
...
def main() -> int:
    ...
    control_models: dict[str, SGDClassifier] = {}
    ...
    for label in TX.CONTROL_DEFICIENCIES:
        ...
        if positives < MIN_POSITIVES:
            ...
            continue
        model = SGDClassifier(loss="log_loss", alpha=3e-6, class_weight="balanced", max_iter=500, tol=1e-2, random_state=RANDOM_STATE)
    ...
    if len(np.unique(sif_target)) >= 2:
        sif_model = SGDClassifier(loss="log_loss", alpha=5e-6, class_weight="balanced", max_iter=600, tol=1e-2, random_state=RANDOM_STATE)
